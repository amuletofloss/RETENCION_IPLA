...
if(outputIndex<0||!process.argv[outputIndex+1])throw new Error('Use --output .sies-work/<workspace>; nunca se escribe sobre el repositorio canónico');
const root=path.resolve(process.argv[outputIndex+1]);
const allowedRoot=path.join(sourceRoot,'.sies-work')+path.sep;
if(!root.startsWith(allowedRoot)||root.includes(`${path.sep}.git${path.sep}`))throw new Error(`Destino inseguro: ${root}`);
if(!fs.existsSync(path.join(root,'.sies-workspace.json')))throw new Error('Workspace sin sentinel; ejecute siespbip prepare antes de construir');
for(const name of ['Retencion_Academica_SIES.pbip','Retencion_Academica_SIES.Report','Retencion_Academica_SIES.SemanticModel']){
...
const data=path.resolve(process.env.SIES_DERIVED_ROOT||path.join(sourceRoot,'.sies-local','derived','retencion_academica_sies'));
const runtimeDataPath=data.replaceAll('"','""');
if(!fs.existsSync(path.join(data,'metadata.json')))throw new Error(`Falta materialización local: ${data}`);

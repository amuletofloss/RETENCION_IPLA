...
def discover_sources(source_root: Path, source: str, years: list[int]) -> list[Path]:
    ...
    if silver.exists():
        ...
        files = sorted(
            ...
            and source in fold(path.as_posix())
            and source_year(path) in years
        )
    grouped = {year: [path for path in files if source_year(path) == year] for year in years}
    duplicates = {year: paths for year, paths in grouped.items() if len(paths) > 1}
    if duplicates:
        detail = ", ".join(f"{year}={len(paths)}" for year, paths in duplicates.items())
        raise FileExistsError(f"Hay fuentes duplicadas de {source}: {detail}")
    found = {year: paths[0] for year, paths in grouped.items() if paths}
...
def create_source_view(con: duckdb.DuckDBPyConnection, name: str, files: list[Path]) -> None:
    ...
    for path in files:
        ...
        if year is None:
            raise ValueError(f"No se pudo determinar el año de {path}")
        selects.append(
...
def materialize(source_root: Path, output: Path) -> dict:
    ...
    metadata = {
        "version_metodologica": "3.3.1",
        "publication_mode": "external_mrun",
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output", type=Path, default=Path(".sies-local/derived/retencion_academica_sies"))

...
REQUIRED = [
    ...
    "docs/12-estandar-reutilizable.md",
    ".github/CODEOWNERS",
...
def markdown_files():
    excluded = {".pbi", ".venv", ".sies-local", ".sies-work"}
    return [path for path in ROOT.rglob("*.md") if not excluded.intersection(path.parts)]
...
def test_manifiesto_documental_y_formularios_yaml_son_validos():
    ...
    assert manifest["project_version"] == "3.3.1"
    assert len(manifest["canonical_documents"]) == 13

...
| `datos/` | Catálogo, hashes y controles agregados públicos | Solo mediante `publish-data` |
| `.sies-local/` | Fuentes y derivados con MRUN | Local e ignorado |
| `.sies-work/` | Copia de edición en Power BI Desktop | Solo mediante `prepare`/`capture` |
...
| `.platform` | [OK] | presente en reporte y modelo semántico |
...
- Los identificadores de página/visual deben ser únicos; la prueba exige 73 identificadores sin colisiones.
...
Se versionan el `.pbip`, `.Report`, `.SemanticModel`, controles agregados, catálogo de fuentes, scripts, pruebas, documentos y automatización. No se versionan fuentes SIES, derivados con MRUN, estado local, secretos ni resultados temporales.
